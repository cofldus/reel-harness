...
class FableService:
    ...
    def delete_character(self, character_id: str) -> str:
        """Removes a character from its project. Returns the project id.

        Refused while ANY shot still names the character as its subject:
        deleting one leaves those shots pointing at an actor that does not
        exist, and the prompt compiler would then quietly generate them
        with no fixed identity at all -- a film whose lead changes face,
        produced without a single error along the way.

        Reference images already on disk are deliberately NOT deleted.
        They cost real money, and this project's rule throughout is that
        paid artifacts are kept as evidence rather than tidied away.
        """
        with self._session_factory() as session:
            character = session.get(FableCharacter, character_id)
            if character is None:
            ...
            using = [
                shot.shot_order for shot in self._shots_for_project(session, project_id)
                if shot.subject == character.name
            ]
            if using:
                raise InvalidActionError(
                    f"{character.name!r} 배우는 샷 {', '.join(str(n) for n in using)}번이 "
                    f"사용 중이라 삭제할 수 없습니다. 먼저 각색을 다시 실행하세요."
                )
            session.delete(character)
            session.commit()
            return project_id
    ...
    def _assert_generation_affordable(
        ...
        try:
            ...
            raise InvalidActionError(str(exc)) from exc
        # Can this provider generate this plan AT ALL? Checked before the
        # budget, and regardless of whether one is set: a shot the provider
        # cannot produce is not a money problem, and discovering it in the
        # worker means every shot fails one at a time instead of the
        # operator being told once, here, for free.
        for shot in shots:
            try:
                resolve_parameters(
                ...
                raise InvalidActionError(
                    f"shot {shot.shot_order} cannot be generated by "
    ...
    def deselect_take(self, take_id: str) -> FableShot:
        """Undoes a take selection, returning the shot to review.

        Needed because selecting is a judgement, and judgements get
        revised -- before this, choosing a take was permanent from the UI
        even though nothing about the domain required that. Refused once
        the project has moved PAST take review: the film has been cut by
        then, and silently un-choosing a shot that is already in it would
        make the rendered film disagree with the project."""
        with self._session_factory() as session:
            take = session.get(FableTake, take_id)
            if take is None:
                raise FableProjectNotFoundError(take_id)
            if not take.selected:
                raise InvalidActionError("이 테이크는 선택된 상태가 아닙니다.")
            shot = session.get(FableShot, take.shot_id)
            ...
            if project.status not in (
                FableProjectStatus.TAKE_REVIEW.value, FableProjectStatus.EDITING.value,
            ):
                raise InvalidActionError(
                    f"프로젝트가 {project.status} 상태라 선택을 되돌릴 수 없습니다."
                )
            take.selected = False
            apply_shot_transition(shot, FableShotStatus.REVIEW_REQUIRED)
            # A project that had advanced to EDITING because every shot was
            # chosen must step back: it no longer is.
