...
def _ready_project(session_factory, tmp_path):
...
def test_lease_next_shot_claims_ready_and_mints_token(session_factory, tmp_path) -> None:
    project = _ready_project(session_factory, tmp_path)
...
def test_second_worker_cannot_lease_a_locked_shot_twice(session_factory, tmp_path) -> None:
    _ready_project(session_factory, tmp_path)
    with session_factory() as session:
...
def test_assert_shot_lease_fences_a_rotated_token(session_factory, tmp_path) -> None:
    _ready_project(session_factory, tmp_path)
    with session_factory() as session:
        shot = lease_next_shot(session, worker_id="fable-a")
...
def test_release_keeps_lease_while_shot_is_active(session_factory, tmp_path) -> None:
    _ready_project(session_factory, tmp_path)
    with session_factory() as session:
        shot = lease_next_shot(session, worker_id="fable-a")
...
def test_release_clears_lease_for_non_active_shot(session_factory, tmp_path) -> None:
    _ready_project(session_factory, tmp_path)
    with session_factory() as session:
        shot = lease_next_shot(session, worker_id="fable-a")
        token = shot.lease_token
...
def test_recover_stale_shots_requeues_within_crash_budget(session_factory, tmp_path) -> None:
    _ready_project(session_factory, tmp_path)
    with session_factory() as session:
...
def test_recover_stale_shots_fails_after_budget_exhausted(session_factory, tmp_path) -> None:
    _ready_project(session_factory, tmp_path)
    with session_factory() as session:
        shot = lease_next_shot(session, worker_id="fable-crashed")
        shot.status = "GENERATING"
...
def test_healthy_heartbeat_prevents_recovery(session_factory, tmp_path) -> None:
    _ready_project(session_factory, tmp_path)
    with session_factory() as session:
...
def test_find_orphaned_active_shots_detects_forbidden_state(session_factory, tmp_path) -> None:
    _ready_project(session_factory, tmp_path)
    with session_factory() as session:
        shot = lease_next_shot(session, worker_id="fable-a")
        shot.status = "GENERATING"
...
def test_project_advances_to_take_review_only_when_no_shot_is_pending(session_factory, tmp_path) -> None:
    project = _ready_project(session_factory, tmp_path)
    with session_factory() as session:
