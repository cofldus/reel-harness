"""publication-reconcile: CLI (single + --all) and the API endpoint wiring.
Core decision logic is covered by tests/unit/test_publish_reconciliation.py;
this file only proves the CLI/API surface calls it correctly. FakePublisher
only, no network."""
...
def _faststart_mp4_bytes(tmp_path, seed: str) -> bytes:
    ...
    out = tmp_path / f"reconcile-cli-{seed}.mp4"
    argv = [
...
def _make_published_publication(job_service, channel, session_factory, storage, tmp_path, key: str) -> Publication:
...
def _isolate(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("DATABASE_URL", f"sqlite:///{(tmp_path / 'reconcile-cli.db').as_posix()}")
    monkeypatch.setenv("JOBS_DIR", str(tmp_path / "jobs"))
    monkeypatch.setenv("REEL_HARNESS_CREDENTIAL_DIR", str(tmp_path.parent / f"{tmp_path.name}-secrets"))
    monkeypatch.chdir(tmp_path)


def test_publication_reconcile_not_found(monkeypatch, tmp_path, capsys) -> None:
    _isolate(monkeypatch, tmp_path)
    exit_code = cli_main.main(["publication-reconcile", "does-not-exist"])
    payload = json.loads(capsys.readouterr().out)
    assert exit_code == 1
    assert payload["error"] == "not found"


def test_publication_reconcile_no_args_refuses(monkeypatch, tmp_path, capsys) -> None:
    _isolate(monkeypatch, tmp_path)
    assert cli_main.main(["publication-reconcile"]) == 2
    assert "usage" in capsys.readouterr().err


def test_publication_reconcile_already_consistent_via_cli(monkeypatch, tmp_path, capsys) -> None:
    ...
    engine = create_engine_from_url(f"sqlite:///{(tmp_path / 'reconcile-cli.db').as_posix()}")
    init_db(engine)
    ...
    pub = _make_published_publication(job_service, channel, factory, storage, tmp_path, "reconcile-cli-1")
    ...
    store = FileSecretStore(tmp_path.parent / f"{tmp_path.name}-secrets", repo_root=tmp_path.parent / "unrelated")
    ...
    _isolate(monkeypatch, tmp_path)
    exit_code = cli_main.main(["publication-reconcile", pub.id])
    payload = json.loads(capsys.readouterr().out)
    assert exit_code == 0
    assert payload["outcome"] == "already_consistent"


def test_publication_reconcile_all_with_nothing_active_returns_empty(monkeypatch, tmp_path, capsys) -> None:
    _isolate(monkeypatch, tmp_path)
    exit_code = cli_main.main(["publication-reconcile", "--all"])
    payload = json.loads(capsys.readouterr().out)
    assert exit_code == 0
    assert payload == []
...
def _make_ctx(tmp_path) -> AppContext:
    settings = Settings(
        database_url=f"sqlite:///{tmp_path / 'reconcile-api.db'}",
...
def test_api_reconcile_not_found(tmp_path) -> None:
    ctx = _make_ctx(tmp_path)
    app.dependency_overrides[get_context] = lambda: ctx
    try:
        client = TestClient(app)
        response = client.post(
            "/v1/publications/does-not-exist/reconcile", headers={"Authorization": "Bearer test-key"},
        )
...
def test_api_reconcile_requires_auth(tmp_path) -> None:
    ctx = _make_ctx(tmp_path)
    app.dependency_overrides[get_context] = lambda: ctx
    try:
        client = TestClient(app)
        response = client.post("/v1/publications/does-not-exist/reconcile")
        assert response.status_code == 401
    finally:
        app.dependency_overrides.clear()


def test_api_reconcile_already_consistent(tmp_path) -> None:
    ctx = _make_ctx(tmp_path)
    channel = ctx.jobs.create_channel(name="c", niche="n", language="en")
    pub = _make_published_publication(
        JobService(ctx.session_factory, storage=ctx.storage), channel, ctx.session_factory, ctx.storage,
        tmp_path, "reconcile-api-1",
    )
    bundle = ctx.bundle_for_publication(pub)
    with ctx.session_factory() as session:
        db_pub = session.get(Publication, pub.id)
        run_publication(session, db_pub, ctx.storage, bundle)
        assert db_pub.status == PublicationStatus.PUBLISHED.value
    ...
    try:
        client = TestClient(app)
        response = client.post(
            f"/v1/publications/{pub.id}/reconcile", headers={"Authorization": "Bearer test-key"},
        )
        assert response.status_code == 200
        assert response.json()["outcome"] == "already_consistent"
    finally:
        app.dependency_overrides.clear()
